...
bool QuestAction::CompleteQuest(Player* player, uint32 entry)
{
    ...
    {
        LOG_INFO("玩家机器人", "{} => 任务 [ {} ] 完成", bot->GetName(), pQuest->GetTitle());
...
bool QuestUpdateCompleteAction::Execute(Event event)
{
    ...
    p.print_storage();
    LOG_INFO("玩家机器人", "Packet: empty{} questId{}", p.empty(), questId);
    ...
    if (qInfo)
    {
        ...
        if (botAI->HasStrategy("debug quest", BotState::BOT_STATE_NON_COMBAT) || botAI->HasStrategy("debug rpg", BotState::BOT_STATE_COMBAT))
        {
            LOG_INFO("玩家机器人", "{} => 任务 [ {} ] 完成", bot->GetName(), qInfo->GetTitle());
            bot->Say("任务 [ " + format + " ] 完成", LANG_UNIVERSAL);
        }
...
bool QuestUpdateAddKillAction::Execute(Event event)
{
    ...
    else
    {
        ...
        {
            LOG_INFO("玩家机器人", "{} => {}", bot->GetName(), BOT_TEXT2("%available/%required: %quest_id", placeholders));
            botAI->Say(BOT_TEXT2("%available/%required for questId: %quest_id", placeholders));
        }

        botAI->TellMasterNoFacing(BOT_TEXT2("%available/%required: %quest_id", placeholders));
    }
    ...
    if (itemPrototype)
    {
        ...
        for (const auto& pair : botAI->GetCurrentQuestsRequiringItemId(itemId))
        {
            ...
            if (botAI->HasStrategy("debug quest", BotState::BOT_STATE_COMBAT) || botAI->HasStrategy("debug quest", BotState::BOT_STATE_NON_COMBAT))
            {
                ...
                botAI->Say(text);
                LOG_INFO("玩家机器人", "{} => {}", bot->GetName(), text);
            }
...
bool QuestUpdateFailedTimerAction::Execute(Event event)
{
    ...
    if (qInfo)
    {
        ...
        botAI->TellMaster(BOT_TEXT2("计时器 %quest_link, 放弃任务", placeholders));
        BroadcastHelper::BroadcastQuestUpdateFailedTimer(botAI, bot, qInfo);
    }
    else
    {
        botAI->TellMaster("计时失败 " + std::to_string(questId));
    }
